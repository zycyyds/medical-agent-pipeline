...
# 导入所有工具
from tools_v2.explore_tools import (
    detect_input_type, get_csv_full_info, list_directory,
    read_csv_sample, read_text_sample, scan_directory, read_excel_sample,
    read_jsonl_sample,
)
from tools_v2.ocr_tools import ocr_and_clean, ocr_image, preprocess_text
from tools_v2.extract_tools import extract_from_text, standardize_entities, cluster_entity_names, ENTITY_SUBFIELDS
from tools_v2.io_tools import (
...
class ReactMedicalAgent(AgentBase):
    ...
    def _get_sem(self) -> asyncio.Semaphore:
        """懒初始化 semaphore（必须在 event loop 内调用）。"""
    ...
    async def _run_directory(self, dir_path: str, plan: ProcessingPlan) -> Dict[str, Any]:
        ...
        # ── 表1：admission_wide.csv（一次住院一行，结构化 + 标准化实体列）────
        # 先收集全部实体名称，LLM 聚类得到标准名映射，再构建宽表
        wide_rows: List[Dict] = []
        wide_cols: list = []
        seen_wide: set = set()

        def _wcol(col: str) -> None:
            if col not in seen_wide:
                seen_wide.add(col)
                wide_cols.append(col)

        # 收集所有实体，按 category 分组名称，做 LLM 聚类
        all_ents_flat: List[Dict] = []
        for pdata in all_results["patients"].values():
            all_ents_flat.extend(pdata.get("merged_entities", []))

        from collections import defaultdict as _dd
        names_by_cat: Dict[str, List[str]] = _dd(list)
        for e in all_ents_flat:
            cat = e.get("category", "")
        ...
        if self.verbose:
            total_names = sum(len(set(v)) for v in names_by_cat.values())
            print(f"[{self.name}] [Dir] 实体名称聚类: {total_names} 个唯一名称，{len(names_by_cat)} 个类别")
        canonical_map: Dict[str, str] = await cluster_entity_names(dict(names_by_cat))

        def _to_col_key(source_file: str, category: str, name: str) -> str:
            stem = os.path.splitext(source_file)[0]
            prefix = stem.replace("_notes", "")
            canon = canonical_map.get(name.strip().lower(), name.strip().lower())
            safe = canon.replace(" ", "_").replace("/", "_").replace("-", "_")
            return f"{prefix}_{category}_{safe}"

        # ── 表2：entities_long.csv（一个实体一行）────────────────────────
        long_rows: List[Dict] = []

        for pid, pdata in all_results["patients"].items():
            sd = pdata.get("structured_data", {})
            ents = pdata.get("merged_entities", [])
            impressions = pdata.get("impressions", [])

            # 患者基本信息
            pt = sd.get("patients", [{}])[0]
            patient_base = {
                "subject_id": pid,
                "gender":     pt.get("gender", ""),
                "anchor_age": pt.get("anchor_age", ""),
                "dod":        pt.get("dod", ""),
            }
            for c in patient_base:
                _wcol(c)

            from collections import defaultdict

            # 按 hadm_id 索引结构化数据
            diag_by_hadm: Dict[str, List[str]] = defaultdict(list)
            for r in sd.get("diagnoses_icd", []):
                diag_by_hadm[r.get("hadm_id", "")].append(r.get("icd_code", ""))

            lab_cnt: Dict[str, int] = defaultdict(int)
            lab_abn: Dict[str, int] = defaultdict(int)
            for r in sd.get("labevents", []):
                h = r.get("hadm_id", "")
                lab_cnt[h] += 1
                if str(r.get("flag", "") or "").strip():
                    lab_abn[h] += 1

            rx_by_hadm: Dict[str, List[str]] = defaultdict(list)
            for r in sd.get("prescriptions", []):
                drug = str(r.get("drug", "") or "").strip()
                if drug:
                    rx_by_hadm[r.get("hadm_id", "")].append(drug)

            icu_by_hadm: Dict[str, List[str]] = defaultdict(list)
            for r in sd.get("icustays", []):
                icu_by_hadm[r.get("hadm_id", "")].append(
                    r.get("first_careunit", "")
                )

            # impression 按 hadm_id 索引
            imp_by_hadm: Dict[str, List[str]] = defaultdict(list)
            for imp in impressions:
                imp_by_hadm[imp.get("hadm_id", "")].append(imp.get("impression", ""))

            # 确定行集合
            admissions = sd.get("admissions", [])
            row_defs = [(r.get("hadm_id", ""), r) for r in admissions] if admissions else [("", {})]

            for hadm_id, adm_row in row_defs:
                # ── 宽表行 ──────────────────────────────────────────────
                row: Dict = {**patient_base}

                for col in ("hadm_id", "admittime", "dischtime", "deathtime",
                            "admission_type", "admission_location",
                            "discharge_location", "insurance",
                            "language", "marital_status", "race"):
                    row[col] = adm_row.get(col, hadm_id if col == "hadm_id" else "")
                    _wcol(col)

                codes = diag_by_hadm.get(hadm_id, [])
                row["diagnoses_count"] = len(codes)
                row["diagnoses_icd_codes"] = ", ".join(codes)
                _wcol("diagnoses_count"); _wcol("diagnoses_icd_codes")

                row["labevents_count"] = lab_cnt.get(hadm_id, 0)
                row["labevents_abnormal_count"] = lab_abn.get(hadm_id, 0)
                _wcol("labevents_count"); _wcol("labevents_abnormal_count")

                drugs = list(dict.fromkeys(rx_by_hadm.get(hadm_id, [])))
                row["prescriptions_count"] = len(rx_by_hadm.get(hadm_id, []))
                row["prescriptions_drugs"] = ", ".join(drugs[:30])
                _wcol("prescriptions_count"); _wcol("prescriptions_drugs")

                icu_units = icu_by_hadm.get(hadm_id, [])
                row["icu_count"] = len(icu_units)
                row["icu_careunit"] = ", ".join(dict.fromkeys(icu_units))
                _wcol("icu_count"); _wcol("icu_careunit")

                row["impressions"] = " | ".join(imp_by_hadm.get(hadm_id, []))
                _wcol("impressions")

                # ── 标准化实体列：{source_prefix}_{category}_{canonical_name}_{subfield} ──
                ent_cell: Dict[str, List[str]] = defaultdict(list)
                for e in ents:
                    e_hadm = e.get("hadm_id", "")
                    if hadm_id and e_hadm and e_hadm != hadm_id:
                        continue
                    cat = e.get("category", "")
                    base_key = _to_col_key(
                        e.get("source_file", ""),
                        cat,
                        e.get("name", ""),
                    )
                    subfields = ENTITY_SUBFIELDS.get(cat, ["value"])
                    for sf in subfields:
                        v = str(e.get(sf, "") or "").strip()
                        if v:
                            ent_cell[f"{base_key}_{sf}"].append(v)

                for col_key, vals in ent_cell.items():
                    row[col_key] = " | ".join(dict.fromkeys(vals))
                    _wcol(col_key)
            ...
            for e in ents:
                ...
                long_row: Dict = {
                    "subject_id":  pid,
                    "hadm_id":     e.get("hadm_id", ""),
                    "note_source": e.get("source_file", ""),
                    "note_id":     e.get("row_id", ""),
                    "category":    cat,
                    "name":        e.get("name", ""),
                }
                for sf in ENTITY_SUBFIELDS.get(cat, ["value"]):
                    long_row[sf] = e.get(sf, "")
                long_rows.append(long_row)

        admission_wide_csv = os.path.join(out_dir, f"admission_wide_{ts}.csv")
        for row in wide_rows:
            for col in wide_cols:
                if col not in row:
                    row[col] = ""
        save_rows_to_csv(wide_rows, admission_wide_csv, columns=wide_cols)
        all_results["output_admission_wide_csv"] = admission_wide_csv

        # entities_long 列：固定头 + 所有 category 子字段的并集
        all_subfields: List[str] = []
        seen_sf: set = set()
        for sfs in ENTITY_SUBFIELDS.values():
            for sf in sfs:
                if sf not in seen_sf:
                    seen_sf.add(sf)
                    all_subfields.append(sf)
        entities_long_csv = os.path.join(out_dir, f"entities_long_{ts}.csv")
        long_cols = ["subject_id", "hadm_id", "note_source", "note_id", "category", "name"] + all_subfields
        save_rows_to_csv(long_rows, entities_long_csv, columns=long_cols)
        all_results["output_entities_long_csv"] = entities_long_csv
    ...
    async def _extract_from_csv(
        ...
        async def _process_row(row: Dict) -> tuple:
            row_id = str(row.get("note_id", row.get("dicom_id", row.get("study_id", "")))).strip()
            raw_hadm = str(row.get("hadm_id", "") or "").strip()
            hadm_id = raw_hadm.split(".")[0] if raw_hadm else ""
            row_ents: List[Dict] = []
            row_imps: List[Dict] = []
            ...
            return row_ents, row_imps

        row_results = await asyncio.gather(*[_process_row(row) for row in rows])
        for row_ents, row_imps in row_results:
            patient_result["merged_entities"].extend(row_ents)
            patient_result.setdefault("impressions", []).extend(row_imps)
