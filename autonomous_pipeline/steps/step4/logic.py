...
from autonomous_pipeline.config import DEFAULT_OUTPUT_ROOT, PROJECT_ROOT, get_agent_config
...
def get_default_output_root() -> str:
    return str(DEFAULT_OUTPUT_ROOT / "step4_results")
...
def resolve_step4_input(input_path: str | Path) -> dict[str, Any]:
    ...
    if not resolved.is_file():
        raise FileNotFoundError(f"Step4 输入表不存在: {resolved}")
    suffix = resolved.suffix.lower()
    if suffix not in TABULAR_SUFFIXES:
        raise FileNotFoundError(f"Step4 输入不是支持的表格文件: {resolved}")
...
def profile_step4_table(
    ...
    if emit:
        emit(f"[Step4][Runtime] ColumnProfiler 开始: rows={len(df)}, columns={total}, workers={workers}")
    ...
    if emit:
        emit(f"[Step4][Runtime] ColumnProfiler 完成: {profile_path}")
    return {
        "input_csv_path": str(input_csv),
...
def classify_step4_column_risks(
    input_csv_path: str | Path,
    ...
    total = len(profiles)
    if emit:
        emit(f"[Step4][Runtime] ColumnRiskClassifier 开始: columns={total}, workers={workers}")
    ...
    if emit:
        emit(f"[Step4][Runtime] ColumnRiskClassifier 完成: low={counts['low']}, medium={counts['medium']}, high={counts['high']}")
    return {
        "input_csv_path": str(input_csv),
...
def run_step4_data_cleaning(
    ...
    if emit:
        emit(
            "[Step4][Runtime] DataCleaningWorker 开始: "
    ...
    if list(cleaned.columns) != list(df.columns) or len(cleaned) != len(df):
        raise ValueError("Step4 清洗后行数或列结构发生变化，已阻止写出。")
    ...
    if emit:
        emit(f"[Step4][Runtime] DataCleaningWorker 完成: changed_cells={changed_cells}")
    return {
        "input_csv_path": str(input_csv),
        "output_root": str(output),
        "cleaned_csv_path": str(cleaned_csv),
...
def validate_step4_output(
    ...
    if not input_csv.is_file():
        issues.append(f"Step4 输入表不存在: {input_csv}")
    if not cleaned_csv.is_file():
        issues.append(f"Step4 cleaned CSV 不存在: {cleaned_csv}")
    ...
    if input_csv.is_file() and cleaned_csv.is_file():
        try:
            ...
            if len(input_df) != len(output_df):
                issues.append(f"Step4 清洗后行数变化: {len(input_df)} -> {len(output_df)}")
            if list(input_df.columns) != list(output_df.columns):
                issues.append("Step4 清洗后列名或列顺序变化。")
        except Exception as exc:
            issues.append(f"Step4 cleaned CSV 无法读取或对账: {exc}")
    ...
    for label, value in (
        ...
        if not path.is_file():
            issues.append(f"Step4 产物不存在: {path}")
...
def run_step4_pipeline(
    ...
    profile = profile_step4_table(input_info["input_csv_path"], output_root=output_root, workers=workers, emit=emit)
    risks = classify_step4_column_risks(
    ...
    cleaned = run_step4_data_cleaning(
        input_info["input_csv_path"],
...
def append_tool_trace(output_root: str | Path, tool: str, arguments: dict[str, Any], payload: dict[str, Any], started_at: float) -> str:
    root = resolve_path(output_root) / "_meta"
...
def _clean_high_risk_column(
    ...
    if not enable_llm:
        method = "llm_disabled_noop"
        status = "noop_failed"
        explanation = f"LLM disabled; {risk_label} column kept unchanged."
        ...
        if cleaner_info is None:
            method = "llm_cleaner_generation_failed_noop"
            status = "noop_failed"
            explanation = f"LLM cleaner generation failed; {risk_label} column kept unchanged."
            _write_noop_cleaner(cleaner_dir, column_name, explanation)
            cleaned_series = original.copy()
            warnings.append(
                f"LLM {risk_label} cleaner generation failed for {column_name}; kept original values: "
            ...
            if runtime_errors:
                method = "llm_cleaner_runtime_failed_noop"
                status = "noop_failed"
                cleaned_series = original.copy()
                warnings.append(
                    f"LLM {risk_label} cleaner runtime failed for {column_name}; kept original values: "
...
def _call_high_risk_llm(prompt: str) -> str:
    ...
    cfg = get_agent_config("step4")
    api_key = os.environ.get("OPENAI_API_KEY") or cfg.api_key
    if not api_key:
        raise RuntimeError("agent_5 api_key is not configured")
    base_url = os.environ.get("OPENAI_API_BASE") or cfg.base_url or "https://api.openai.com/v1"
    model = os.environ.get("MODEL_NAME") or cfg.model or "gpt-4.1-mini"
    timeout = float(cfg.timeout or 120)
...
def _infer_high_risk_column_profile(column_name: str, values: list[str], risk_item: dict[str, Any]) -> dict[str, Any]:
    ...
    elif any(token in lower for token in ["time", "date", "日期", "时间"]) or ratio(is_dt) >= 0.6:
        column_type, reason = "datetime", "列名或样本像时间列"
    elif any(token in lower for token in ["code", "icd", "编码", "代码"]):
        column_type, reason = "code", "列名像编码列"
    elif ratio(is_num) >= 0.8:
        column_type, reason = "numeric", "大部分样本是数值"
    elif len(set(values[:20])) <= 10 and len(values) >= 3:
        column_type, reason = "categorical", "样本重复度高"
    elif ratio(lambda value: len(value) >= 40) >= 0.5:
        column_type, reason = "free_text", "样本较长"
    else:
...
def _emit_progress(label: str, done: int, total: int, emit: ProgressCallback | None) -> None:
    ...
    if done == 1 or done == total or done % step == 0:
        emit(f"[Step4][Progress] {label}: {done}/{total} ({done / total * 100:.1f}%)")
