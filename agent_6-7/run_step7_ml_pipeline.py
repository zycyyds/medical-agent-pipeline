...
if _IS_MIMIC:
    ...
    SELECTION_REPORT = _find_latest_by_suffix("_selection_report", ".json")
...
def _load_passed_patient_ids() -> list[str]:
    """
    从 output_step6/ 下最新的 passed_patients_<ts>.json 读取通过患者 ID 列表。
    若文件不存在，回退到从 consistency_report_*.txt 解析，
    最终回退到全部患者。
    """
    # 优先读 passed_patients_*.json（由 run_step6 的 save_step6_report 写入）
    if os.path.isdir(OUTPUT_STEP6):
        passed_files = sorted(
            f for f in os.listdir(OUTPUT_STEP6)
            if f.startswith("passed_patients_") and f.endswith(".json")
        )
        if passed_files:
            latest = os.path.join(OUTPUT_STEP6, passed_files[-1])
            try:
                with open(latest, encoding="utf-8") as fh:
                    data = json.load(fh)
                ids = data.get("passed_patient_ids", [])
                if ids:
                    print(f"[Step 7] 从 {latest} 读取 {len(ids)} 个通过患者")
                    return [str(i) for i in ids]
            except Exception as exc:
                print(f"[Step 7] 读取 passed_patients 文件失败: {exc}")

        # 回退：从 consistency_report_*.txt 解析
        report_files = sorted(
            f for f in os.listdir(OUTPUT_STEP6)
        ...
        if report_files:
            ...
            try:
                ...
                if m:
                    ...
                    if ids:
                        print(f"[Step 7] 从报告文件解析 {len(ids)} 个通过患者: {latest}")
                        return ids
            except Exception as exc:
                print(f"[Step 7] 读取报告文件失败: {exc}")

    # 最终回退：全部患者
    print("[Step 7] 未找到 Step 6 输出，使用全部患者")
    filt = _read_data(FILTERED_CSV)
    return [str(p) for p in filt["patient_id"].unique()]
...
async def run_step7() -> None:
    ...
    msg7 = Msg(
        ...
        content=(
            ...
            "请执行 Step 7：自主探索数据集，发现 2～4 个可行的 ML 任务，"
            "并为每个任务生成独立的训练数据集。\n\n"
            "工作流：\n"
            "① 调用 load_task_context 了解数据概况和全量标签候选列\n"
            "② 自主判断哪些列适合作为标签（考虑缺失率、类别数、业务含义）\n"
            "③ 对每个任务选择合适策略（非空二分类/ICD二分类/多分类），"
