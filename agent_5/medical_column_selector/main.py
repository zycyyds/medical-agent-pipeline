...
class TaskDrivenColumnSelector:
    ...
    def _force_keep_mimic_core_columns(
        self,
        selection_result: Dict[str, Any],
        schema_profile: Dict[str, Any],
    ) -> Dict[str, Any]:
        """把 MIMIC structured/core 字段加入 must-have，避免被模型漏掉。"""
        ...
        overrides = selection_result.setdefault("mimic_core_overrides", [])

        for col_name in columns.keys():
            if not self._is_mimic_core_column(col_name):
                continue
            ...
            reasons[col_name] = (
                "Forced keep: MIMIC structured/core field needed for patient, "
                "encounter, diagnosis, labs, medication, ICU, or traceability."
            )
            overrides.append({
                "column": col_name,
                "action": "forced_keep_mimic_core",
                "reason": "MIMIC structured/core column",
            })
    ...
    def run(self, input_csv_path: str, task_text: str, output_dir: Optional[str] = None) -> Dict[str, str]:
        ...
        selection_result = self._force_keep_mimic_core_columns(
